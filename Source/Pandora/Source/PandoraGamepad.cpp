...
namespace Killer
{
	static KIL_UINT32 s_CodeToButton[ 256 ] =
	{
		// 0
		0, 0, 0, 0, 0, 0, 0, 0,
		// 8
		...
		0, 0, 0, 0, 0, 0, 0, 0,
		// 48
		0, 0, 0, 0, 0, 0, GAMEPAD_BUTTON_L, 0,
		// 56
		...
		0, 0, 0, 0, 0, 0, 0, 0,
		// 96
		0, GAMEPAD_BUTTON_R, 0, 0, 0, 0, GAMEPAD_BUTTON_A,
	...
	KIL_UINT32 Gamepad::Initialise( )
	{
		...
		KIL_UINT32 Return = KIL_OK;

		if( m_ButtonsFD == -1 )
		{
			std::cout << "[Killer::Gamepad::Initialise] <ERROR> "
		...
		{
			std::cout << "[Killer::Gamepad::Initialise] <ERROR> "
				"Failed to open right analogue stick for reading" << std::endl;
	...
	KIL_UINT32 Gamepad::GetState( void *p_pState )
	{
		if( p_pState == KIL_NULL )
		{
			std::cout << "[Killer::Gamepad::GetState] <ERROR> "
		...
		Return = select( m_MaxFD + 1, &ReadFDS, KIL_NULL, KIL_NULL, &TimeOut );
		...
		for( size_t i = 0; i < Read / sizeof( Event[ 0 ] ); ++i )
		{
			switch( Event[ i ].type )
			{
				case EV_ABS:
				{
					int Stick = ( FD == m_LeftStickFD ) ? 0 : 1;
